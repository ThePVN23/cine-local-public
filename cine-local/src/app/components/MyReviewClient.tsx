...
export default function MyReviewsClient({ user }: MyReviewsClientProps) {
  ...
  const [activeReviewIndex, setActiveReviewIndex] = useState<
  ...
  async function fetchPosters(reviewData: Review[]) {
    ...
    const posterPromises = uniqueMovieIds.map((movieId) =>
      fetch(`https://api.themoviedb.org/3/movie/${movieId}`, {
        ...
        .catch(() => ({ movieId, poster_path: null }))
    );
  ...
  const handleNextReview = (movieTitle: string, totalReviews: number) => {
    setActiveReviewIndex((prev) => ({
      ...prev,
      [movieTitle]:
        (prev[movieTitle] || 0) + 1 >= totalReviews
          ? 0
          : (prev[movieTitle] || 0) + 1,
    }));
  };
  ...
  async function handleDelete(reviewId: string, movieTitle: string) {
    ...
    try {
      ...
      if (res.ok) setReviews((reviews) => reviews.filter((r) => r._id !== reviewId));
    } catch (e) {
          ...
          {Object.entries(groupedReviews).map(([movieTitle, reviewGroup]) => {
            ...
            return (
              ...
              >
                ...
                >
                  <a href={`/movieDetailPage/${latestReview.movieId}`}>
                    <img
                      src={
                        posterPath
                ...
                <div style={{ flexGrow: 1, position: "relative" }}>
                  ...
                  >
                    ...
                    >
                      Your Review{" "}
                      {isMultipleReviews &&
                        ` (${activeIndex + 1} of ${reviewGroup.length})`}
                    </h4>
                    <span
                  ...
                  <div style={{ minHeight: "150px" }}>
                    ...
                    <button
                      onClick={() =>
                        handleDelete(latestReview._id, movieTitle)
                      }
                      style={{
                  ...
                  {isMultipleReviews && (
                    ...
                    >
                      <button
                        onClick={() =>
                          handlePrevReview(movieTitle, reviewGroup.length)
                        }
                        style={{
                          padding: "0.5rem 1rem",
                      ...
                      <button
                        onClick={() =>
                          handleNextReview(movieTitle, reviewGroup.length)
                        }
                        style={{
                          padding: "0.5rem 1rem",
                          backgroundColor: "#374151",
